...
class TiendaModelShippingRatesWeightbased extends TiendaModelBase
{
	protected function _buildQueryWhere(&$query)
	{
		...
		if ( strlen( $filter_id ) )
		{
			$query->where('tbl.shipping_rate_weightbased_id = '.(int) $filter_id);
		}
		if( strlen( $filter_shippingmethod ) )
		{
			$query->where('tbl.shipping_method_weightbased_id = '.(int) $filter_shippingmethod);
		}
		if (strlen($filter_weight))
		{
			$query->where("(
        		tbl.weight_start <= '".$filter_weight."' 
        		AND (
                    tbl.weight_end >= '".$filter_weight."'
                    OR
                    tbl.weight_end = '0.000'
                    )
			)");
		}
	...
	public function getList()
	{
		...
		{
			$item->link_remove = 'index.php?option=com_tienda&task=doTask&elementTask=delete&cid[]='.$item->shipping_rate_weightbased_id;
		}
		return $list;
	}
}
