...
class plgTiendaPayment_cardpay extends TiendaPaymentPlugin
{
	/**
	 * @var $_element  string  Should always correspond with the plugin's filename, 
	 *                         forcing it to be unique 
	 */
    var $_element    = 'payment_cardpay';
    ...
    function _generateSignature( $data , $type  )
    {
    	...
    	switch( $type )
    	{
    		case 1 : // outcoming message
    			...
    			$data['order_id'].$data['cs'].$data['rurl'].$data['ipc'].$data['name'];
    			break;
    		
    		case 2 : // incoming message    		
    			$str = $data['vs'].$data['res'].$data['ac'];
    			break;
    	}
    ...
    function _generateRestUri( $data )
    {
    	...
    	if( strlen( $data['rem'] ) )
    		$url[] = 'REM='.$data['rem'];
    	$url[] = 'DESC='.JText::sprintf( 'CARDPAY_DESC_OUTCOMING_MESSAGE', $data['order_id'] );
    	$url[] = 'AREDIR=1';
