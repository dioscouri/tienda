...
<div>
    ...
    foreach ($attributes as $attribute)
    {
        ...
        if ($item->product_check_inventory == 1) 
        {
            ...
            $attribs = array('class' => 'inputbox', 'size' => '1','onchange'=>"tiendaUpdateAddToCart( 'product','product_buy', this.form );");
    ...
    <div id='add_to_cart' style="display: block;"> 
        ...
        if (empty($item->product_check_inventory) || (!empty($item->product_check_inventory) && empty($this->invalidQuantity)) ) :
            switch (TiendaConfig::getInstance()->get('cartbutton', 'image')) 
            {
                ...
                default:
                    ?> 
                    <img class='addcart' src='<?php echo Tienda::getUrl('images')."addcart.png"; ?>' alt='<?php echo JText::_('COM_TIENDA_ADD_TO_CART'); ?>' onclick="<?php echo $onclick; ?>" />
                    <?php
                    break;
            }
        endif; 
