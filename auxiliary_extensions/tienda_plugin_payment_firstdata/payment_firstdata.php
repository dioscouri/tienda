...
class plgTiendaPayment_firstdata extends TiendaPaymentPlugin
{
    ...
    function _getActionUrl($type = 'simple')
    {
        if ($type == 'simple') 
        {
            $url  = $this->params->get('sandbox') ? 'https://test.FirstData/gateway/transact.dll' : 'https://secure.FirstData/gateway/transact.dll';
        }
            else 
        {
            // recurring billing url
            $url = $this->params->get('sandbox') ? 'https://apitest.FirstData/xml/v1/request.api' : 'https://api.FirstData/xml/v1/request.api';         
        }
    ...
    function _getProcessVars($data)
    {
        ...
        $auth_method                = "CC";                 // or ECHECK
            # ECHECK DATA           // see http://developer.FirstData/guides/AIM/eCheck_Developer_Guide/Transaction_Data_Requirements.htm
    ...
    function _processSimplePayment($authnet_values) 
    {
        ...
        // prepare the array for posting to FirstData
        $fields = '';
    ...
    function _evaluateSimplePaymentResponse( $resp, $submitted_values )
    {
        ...
        if ( ! ($user =& $this->_getUser( $submitted_values ))) {
            $errors[] = JText::_('FIRSTDATA Message Unknown User');
        ...
        // =======================
            ...
            if ($stored_amount != $respond_amount ) {
                $errors[] = JText::_('TIENDA FIRSTDATA MESSAGE PAYMENT AMOUNT INVALID');
            ...
            if (empty($errors))
            {
                $return = JText::_( "TIENDA FIRSTDATA MESSAGE PAYMENT SUCCESS" );
                return $return;                
            }
