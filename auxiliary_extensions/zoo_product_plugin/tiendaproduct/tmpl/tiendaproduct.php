...
<script type="text/javascript" src="<?php $uri = JURI::getInstance(); echo $uri->root(); ?>media/com_tienda/js/tienda.js"></script>
    ...
    <span id="product_price" class="product_price">
        ...
        if (!empty($vars->show_tax))
        {
            if (!empty($vars->tax))
            {
                if ($vars->show_tax == '2')
                {
                    echo TiendaHelperBase::currency($product->price + $vars->tax);
                }
                    else
                {
        ...
        if (TiendaConfig::getInstance()->get( 'display_prices_with_shipping') && !empty($product->product_ships))
        {
