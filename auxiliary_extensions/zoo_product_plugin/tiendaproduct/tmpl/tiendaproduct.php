...
<div id="product_buy_<?php echo $item->product_id; ?>" class="product_buy">
    ...
    <form action="<?php echo JRoute::_( 'index.php?option=com_tienda&controller=products&view=products&id="'.$item->product_id ); ?>" method="post" class="adminform" name="<?php echo $formName; ?>" enctype="multipart/form-data" >
...
<div>
    ...
    <form action="<?php echo JRoute::_( 'index.php?option=com_tienda&controller=products&view=products&id="'.$vars->product_id ); ?>" method="post" class="adminform" name="adminForm_<?php echo @$product->product_id; ?>" enctype="multipart/form-data" >
    ...
    foreach ($attributes as $attribute)
    {
        ...
        if ($product->product_check_inventory == 1) 
        {
        	Tienda::load('TiendaSelect', 'library.select');
    ...
    <div id='add_to_cart' style="display: block;"> 
        ...
        <?php $onclick = "tiendaFormValidation( '".@$vars->validation."', 'validationmessage', 'addtocart', document.adminForm_".$product->product_id." );"; ?>
        
        <?php 
        if (empty($product->product_check_inventory) || (!empty($product->product_check_inventory) && empty($vars->invalidQuantity)) ) :
            switch (TiendaConfig::getInstance()->get('cartbutton', 'image')) 
            {
                case "button":
                    ?>
    ...
    <?php if (!empty($product->product_recurs)) : ?> 
        <div id='product_recurs'> 
            <span class="title"><?php echo JText::_("THIS PRODUCTS CHARGES RECUR"); ?></span>
            <div id="product_recurs_prices"> 
            <?php echo JText::_( "RECURRING PRICE" ); ?>: <?php echo TiendaHelperBase::currency($product->recurring_price); ?>
            (<?php echo $product->recurring_payments . " " . JText::_( "PAYMENTS" ); ?>, <?php echo $product->recurring_period_interval." ". JText::_( "$product->recurring_period_unit PERIOD UNIT" )." ".JText::_( "PERIODS" ); ?>) 
            <?php if ($product->recurring_trial) : ?>
                <br/>
                <?php echo JText::_( "TRIAL PERIOD PRICE" ); ?>: <?php echo TiendaHelperBase::currency($product->recurring_trial_price); ?>
                (<?php echo "1 " . JText::_( "PAYMENT" ); ?>, <?php echo $product->recurring_trial_period_interval." ". JText::_( "$product->recurring_trial_period_unit PERIOD UNIT" )." ".JText::_( "PERIOD" ); ?>)
            <?php endif; ?> 
            </div>
        </div>
    <?php endif; ?>
    
    <?php if (!empty($product->product_check_inventory)) : ?>
        <?php if (TiendaConfig::getInstance()->get('display_product_quantity', '1')) : ?> 
    ...
    <?php if (!empty($product->product_check_inventory) && !empty($vars->invalidQuantity) ) : ?>
        <!-- Not avilable in stock  --->  
        <div id='out_of_stock'> 
          <?php echo JText::_("OUT_OF_STOCK"); ?> 
        </div>
