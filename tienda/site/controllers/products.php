	...
	function validate( )
	{
		...
		foreach ( $values as $key => $value )
		{
			if ( substr( $key, 0, 10 ) == 'attribute_' )
			{
				...
				if(  !( int )@$value )
				{
					$response['msg'] = $helper->generateMessage( JText::_( "COM_TIENDA_ALL_PRODUCT_ATTRIBUTES_ARE_REQUIRED!" ) );
					$response['error'] = '1';
					echo ( json_encode( $response ) );
					return false;
				}
			...
			if ( $product->quantity_restriction )
			{
				...
				if ( $max )
				{
					...
					if ( $product_qty > $remaining )
					{
						...
						$msg = $helper
								->generateMessage( 
										JText::_( "COM_TIENDA_YOU_HAVE_REACHED_THE_MAXIMUM_QUANTITY_FOR_THIS_ITEM_YOU_CAN_ORDER_ANOTHER" ) . " " . $remaining );
					}
				}
				if ( $min )
				{
					if ( $product_qty < $min )
					{
						...
						$msg = $helper
								->generateMessage( 
										JText::_( "COM_TIENDA_YOU_HAVE_NOT_REACHED_THE_MAXIMUM_QUANTITY_FOR_THIS_ITEM_YOU_HAVE_TO_ORDER_ATLEAST" ) . " "
												. $min );
					}
				}
			}
	...
	function validateChildren( )
	{
		...
		if ( $children = $model->getList( ) )
		{
			foreach ( $children as $child )
			{
				...
				{
					$response['msg'] = $helper
							->generateMessage( JText::sprintf( 'NOT_AVAILABLE_QUANTITY', $availableQuantity->product_name, $product_qty ) );
					$response['error'] = '1';
					echo ( json_encode( $response ) );
					return false;
				}
 ...
 continue;
	...
	function validateReview( )
	{
		...
		if ( !$user->id )
		{
			...
			{
				$errors[] = '<li>' . JText::_( "COM_TIENDA_NAME_OF_FIELD_REQUIRED." ) . '</li>';
			}
			...
			{
				$errors[] = '<li>' . JText::_( "COM_TIENDA_YOU_ALREADY_SUBMITTED_REVIEW_YOU_CAN_SUBMIT_A_REVIEW_ONCES." ) . '</li>';
			}
		}
		else
		{
	...
	function addReview( )
	{
		...
		if ( !$user->id )
		{
			...
			{
				$valid = false;
				$this->message = JText::_( "COM_TIENDA_ NAME_FIELD_IS_REQUIRED" );
				$this->messagetype = 'notice';
			}
			...
			{
				$valid = false;
				$this->message = JText::_( "COM_TIENDA_YOU_ALREADY_SUBMITTED_REVIEW_YOU_CAN_SUBMIT_A_REVIEW_ONCES" );
				$this->messagetype = 'notice';
			}
		}
		else
	...
	function sendAskedQuestion( )
	{
		...
		{
			$valid = false;
			$this->message = JText::_( "COM_TIENDA_NAME_OF_FIELD_REQUIRED" );
			$this->messagetype = 'notice';
		}
		...
		if ( $valid )
		{
			...
			{
				$this->message = JText::_( "COM_TIENDA_MESSAGE_SUCCESFULLY_SENT" );
			}
			else
			{
