...
/**
 * HTML Event Element View class for the Content component
 *
 * @package		Joomla
 * @subpackage	Content
 * @since 1.5
 */
class tiendaViewelementevent extends JView
{
	function display()
	{
		global $mainframe;
		...
		$document->setTitle('Event Selection');
...
//		
		...
		$link = 'index.php?option=com_tienda&task=elementEvent&tmpl=component&object='.$object;
		// index.php?option=com_juga&amp;task=elementArticle&amp;tmpl=component&amp;object=articleid
		?>
		<form action="<?php echo $link; ?>" method="post" name="adminForm">
			...
			<thead>
				<tr>
					...
					<th class="title">
						<?php echo JHTML::_('grid.sort',   'Title', 'c.summary' ); ?>
					</th>
				</tr>
			</thead>
			<tfoot>
			...
			for ($i=0, $n=count( $rows ); $i < $n; $i++)
			{
				...
				<tr class="<?php echo "row$k"; ?>">
					...
					<td>
						<a style="cursor: pointer;" onclick="window.parent.jSelectArticle('<?php echo $row->evdet_id; ?>', '<?php echo str_replace(array("'", "\""), array("\\'", ""),$row->summary); ?>', '<?php echo JRequest::getVar('object'); ?>');">
							<?php echo htmlspecialchars($row->summary, ENT_QUOTES, 'UTF-8'); ?></a>
					</td>
