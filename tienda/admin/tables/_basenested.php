...
class TiendaTableNested extends TiendaTable
{
	/**
	 * Extends to add some fields based on calculations
	 * @see tienda/admin/tables/TiendaTable#load($oid, $key)
	 */
	...
	function delete( $id=null ) 
	{
		...
		if (empty($this->$key))
		{
			$this->setError( JText::_('COM_TIENDA_INVALID_ITEM') );
			return false;	
		}
	 ...
	 * @see tienda/admin/tables/TiendaTable#move($change, $where)
	 */
	...
	public function getRoot()
	{
		...
		if ( in_array( 'isroot', array_keys( $this->getProperties() ) ) ) 
		{
				...
				else 
			{
				...
				$this->setError( JText::_('COM_TIENDA_MULTIPLE_ROOTS_DEFINED') );
			}
		}
		...
		if (!$return)
		{
				...
				else
			{
				...
				{
					$this->setError( JText::_('COM_TIENDA_ROOT_NOT_FOUND_BY_UNIQUENESS') );
					return false;
				}
			}			
		}
	...
	public function orderUp($oid=null)
	{
		...
		if (empty($node->$key)) 
		{
			$this->setError( JText::_('COM_TIENDA_COULD_NOT_LOAD_NODE') );
			$this->_unlock();
			return false;
		}
		...
		{
			$this->setError( JText::_('COM_TIENDA_COULD_NOT_LOAD_SIBLING') );
			$this->_unlock();
			return false;
		}
	    if ($sibling->parent_id != $node->parent_id || $sibling->$key == $node->$key) 
        {
            $this->setError( JText::_('COM_TIENDA_NODE_CANNOT_BE_ORDERED_ANY_HIGHER_UP') );
            $this->_unlock();
            return false;
        }
	...
	public function orderDown($oid)
	{
	    ...
	    if ($sibling->parent_id != $node->parent_id || $sibling->$key == $node->$key) 
        {
            $this->setError( JText::_('COM_TIENDA_NODE_CANNOT_BE_ORDERED_ANY_LOWER') );
            $this->_unlock();
            return false;
        }
