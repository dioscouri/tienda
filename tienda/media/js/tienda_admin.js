...
function tiendaUpdateAttributesForm(id) {
    var url = 'index.php?option=com_tienda&view=products&task=tiendaUpdateAttributesForm&protocol=json&product_id=' + id;
    var form = document.adminForm;
    ...
    var a=new Ajax(url,{
        ...
        onComplete: function(response){
            var resp=Json.evaluate(response, false);
            $('current_attributes').setHTML(resp.html);
            return true;
        }
    }).request();
}