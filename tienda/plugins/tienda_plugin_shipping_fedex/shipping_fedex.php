	...
	function sendRequest($address, $orderItems) {
		...
		foreach ($orderItems as $item) {
			...
			if ($product -> product_ships) {
				...
				$weight = array('Value' => round($product_totalWeight, 1), 'Units' => $this -> params -> get('weight_unit', 'KG') // get this from product?
				);

				$dimensions = array('Length' => $product -> product_length, 'Width' => $product -> product_width, 'Height' => $product -> product_height, 'Units' => $this -> params -> get('dimension_unit', 'CM') // get this from product?
				);
