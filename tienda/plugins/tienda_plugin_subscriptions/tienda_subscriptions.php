...
class plgSystemTienda_Subscriptions extends JPlugin {
	...
	function plgSystemTienda_Subscriptions(&$subject, $config) {
		...
		$language -> load('plg_tienda_system' . $this -> _element, JPATH_ADMINISTRATOR, 'en-GB', true);
		$language -> load('plg_tienda_system' . $this -> _element, JPATH_ADMINISTRATOR, null, true);
	}
