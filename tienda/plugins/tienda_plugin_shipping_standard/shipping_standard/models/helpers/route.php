...
class TiendaHelperRoute extends DSCHelperRoute 
{
   ...
   public static function findItemid($needles=array('view'=>'products', 'task'=>'', 'filter_category'=>'', 'id'=>''))
    ...
    static function build( &$query )
    {
        ...
        // in most versions before v1.0, tienda views pass in the current itemid, so this isn't useful yet
        $menu = $app->getMenu();
        ...
        if ( !empty($query['view']) && $query['view'] == 'products') 
        {
            ...
            if ( !empty($query['task']) && $query['task'] == 'view') 
            {
				...
				if (strpos($query['id'], ':') === false) {
					$pmodel = Tienda::getClass('TiendaModelProducts', 'models.products');
