...
class plgTiendaShipping_ups extends TiendaShippingPlugin {
	...
	function sendShipment($order) {
		...
		foreach ($orderItems as $item) {
			...
			if ($product -> product_ships) {
				...
				$weight = array('Weight' => round($product -> product_weight, 1), 'UnitOfMeasurement' => array('Code' => $this -> params -> get('weight_unit', 'KGS')) // get this from product?
				);

				$dimensions = array('Length' => ( int )$product -> product_length, 'Width' => ( int )$product -> product_width, 'Height' => ( int )$product -> product_height, 'UnitOfMeasurement' => array('Code' => $this -> params -> get('dimension_unit', 'CM')) // get this from product?
				);
		...
		$ups -> setOriginName(Tienda::getInstance() -> get('shop_name'));
	...
	function getUpsServices() {
		$services = array('01' => JText::_('COM_TIENDA_UPS_NEXT_DAY_AIR'), '02' => JText::_('COM_TIENDA_UPS_SECOND_DAY_AIR'), '03' => JText::_('COM_TIENDA_UPS_GROUND'), '07' => JText::_('COM_TIENDA_UPS_WORLDWIDE_EXPRESS'), '08' => JText::_('COM_TIENDA_UPS_WORLDWIDE_EXPEDITED'), '11' => JText::_('COM_TIENDA_UPS_STANDARD'), '12' => JText::_('COM_TIENDA_UPS_THREE-DAY_SELECT'), '13' => JText::_('COM_TIENDA_UPS_NEXT_DAY_AIR_SAVER'), '14' => JText::_('COM_TIENDA_UPS_NEXT_DAY_AIR_EARLY_AM'), '54' => JText::_('COM_TIENDA_UPS_WORLDWIDE_EXPRESS_PLUS'), '59' => JText::_('COM_TIENDA_UPS_SECOND_DAY_AIR_AM'), '65' => JText::_('COM_TIENDA_UPS_SAVER'), );
	...
	function sendRequest($address, $orderItems) {
		...
		foreach ($orderItems as $item) {
			...
			if ($product -> product_ships) {
				...
				$weight = array('Weight' => round($product_totalWeight, 1), 'UnitOfMeasurement' => array('Code' => $this -> params -> get('weight_unit', 'KGS')) // get this from product?
				);

				$dimensions = array('Length' => round($product -> product_length, 2), 'Width' => round($product -> product_width, 2), 'Height' => round($product -> product_height, 2), 'UnitOfMeasurement' => array('Code' => $this -> params -> get('dimension_unit', 'CM')) // get this from product?
				);

				$package = array('PackageWeight' => $weight, 'Dimensions' => $dimensions, 'PackagingType' => array('Code' => $this -> params -> get('packaging', '02')), );

				if ($this -> params -> get('declare_value', 1)) {
					$package['PackageServiceOptions']['DeclaredValue'] = array('CurrencyCode' => $this -> params -> get('currency_code', 'USD'), 'MonetaryValue' => $item -> orderitem_final_price, );
				}
	...
	function download($file, $path) {
		...
		switch ( $ext ) {
			...
			case "html" :
				if ($path)
					die("<b>Cannot be used for " . $ext . " files!</b>");
